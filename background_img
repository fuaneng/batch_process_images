...
background_image_path = r'C:\Nathat_work\pchuli\img\background.png'
...
with tqdm(total=total_files, desc="处理进度", unit="file") as pbar:
    ...
    for filename in files:
        ...
        # 调整背景图片大小以匹配输入图片大小
        background_resized = background_img.resize(img.size)
        
        # 将输入图片粘贴到背景图片上
        background_resized.paste(img, (0, 0), img)
        
        # 保存结果图片到输出文件夹
        output_path = os.path.join(output_folder, filename)
        background_resized.save(output_path)
